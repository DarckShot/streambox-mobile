...
В приложение подключён React Navigation: корневой native stack, нижняя панель вкладок и типизированные параметры маршрутов. На Home реализован визуальный каталог на `FlashList` с локальными mock-данными RUTUBE; переход к деталям пока оставлен как callback-контракт без продуктовой логики. Остальные продуктовые маршруты представлены экранами-заглушками, видеоплеер не реализован. `App.tsx` настраивает safe area, системную тему, `StatusBar` и корневой контейнер навигации.
